...
let lang="ar",activeProduct="shampoo",activePlan="complete";
function setLanguage(next){lang=next;document.documentElement.lang=lang;document.documentElement.dir=lang==="ar"?"rtl":"ltr";$("#langToggle").textContent=lang==="ar"?"EN":"ع";$$('[data-i18n]').forEach(el=>{const v=content[lang][el.dataset.i18n];if(v!==undefined)el.innerHTML=v});$$('[data-placeholder-ar]').forEach(el=>el.placeholder=el.dataset[lang==="ar"?"placeholderAr":"placeholderEn"]);renderProduct(activeProduct);renderPlan(activePlan);updateOrder();}
function renderProduct(key){activeProduct=key;const index=["shampoo","conditioner","mask","serum"].indexOf(key)+1,p=products[key][lang];$("#productImage").src=products[key].image;$("#productImage").alt=p.name;$("#productImage").style.animation="none";requestAnimationFrame(()=>$("#productImage").style.animation="productIn .55s ease");$("#productStep").textContent=`STEP 0${index}`;$("#productName").textContent=p.name;$("#productDescription").textContent=p.desc;$("#productBenefit").textContent=p.benefit;$("#productUse").textContent=p.use;$("#ingredientChips").innerHTML=p.ingredients.map(x=>`<span class="chip">${x}</span>`).join("");$$('.product-tab').forEach(b=>{const on=b.dataset.product===key;b.classList.toggle('active',on);b.setAttribute('aria-selected',on)})}
function renderPlan(type){activePlan=type;$$('[data-plan]').forEach(b=>b.classList.toggle('active',b.dataset.plan===type));$("#weekPlan").innerHTML=plans[type][lang].map((d,i)=>`<article class="day"><span class="day-num">0${i+1}</span><div><strong>${d[0]}</strong><p>${d[1]}</p></div></article>`).join("")}
