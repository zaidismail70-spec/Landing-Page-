...
function setLanguage(next,options){
  ...
  const cartToggleBtn=$("#cartToggle");if(cartToggleBtn)cartToggleBtn.setAttribute("aria-label",content[lang].openCart);
  const cartCloseBtn=$("#cartClose");if(cartCloseBtn)cartCloseBtn.setAttribute("aria-label",content[lang].closeCart);
  ...
  renderProduct(activeProduct);renderPlan(activePlan);renderSingles();renderCart();syncHero();
}
...
const PRICES={package:{complete:30,duo:20},single:{shampoo:13,conditioner:13,mask:15,serum:15}};
const PACKAGE_META={
  complete:{ar:{name:"بكج بلازما الرباعي"},en:{name:"PLASMA Complete Set"},image:"assets/complete.webp"},
  duo:{ar:{name:"بكج بلازما الثنائي"},en:{name:"PLASMA Duo Set"},image:"assets/duo.webp"}
};
function loadCart(){
  try{
    const raw=localStorage.getItem("betolla-cart");
    if(!raw)return [];
    const parsed=JSON.parse(raw);
    if(!Array.isArray(parsed))return [];
    return parsed.filter(it=>it&&(it.kind==="package"||it.kind==="single")&&PRICES[it.kind]&&PRICES[it.kind][it.key]&&Number.isFinite(it.qty)&&it.qty>0);
  }catch(e){return [];}
}
let cart=loadCart();
function saveCart(){try{localStorage.setItem("betolla-cart",JSON.stringify(cart));}catch(e){}}
function cartUnitPrice(kind,key){return PRICES[kind][key]}
function cartItemName(kind,key){return kind==="package"?PACKAGE_META[key][lang].name:products[key][lang].name}
function cartItemImage(kind,key){return kind==="package"?PACKAGE_META[key].image:products[key].image}
function addToCart(kind,key,qty){
  qty=qty||1;
  const existing=cart.find(it=>it.kind===kind&&it.key===key);
  if(existing)existing.qty=Math.min(20,existing.qty+qty);
  else cart.push({kind,key,qty:Math.min(20,qty)});
  saveCart();renderCart();openCartDrawer();
}
function setCartQty(kind,key,qty){
  const existing=cart.find(it=>it.kind===kind&&it.key===key);
  if(!existing)return;
  if(qty<=0)cart=cart.filter(it=>it!==existing);
  else existing.qty=Math.min(20,qty);
  saveCart();renderCart();
}
function removeFromCart(kind,key){cart=cart.filter(it=>!(it.kind===kind&&it.key===key));saveCart();renderCart();}
function cartTotals(){
  const subtotal=cart.reduce((sum,it)=>sum+cartUnitPrice(it.kind,it.key)*it.qty,0);
  const delivery=cart.length?3:0;
  return {subtotal,delivery,total:subtotal+delivery,count:cart.reduce((n,it)=>n+it.qty,0)};
}
function cartRowHtml(it){
  const price=cartUnitPrice(it.kind,it.key);
  return `<div class="cart-item"><img src="${cartItemImage(it.kind,it.key)}" alt="" width="60" height="60"><div><p class="cart-item-name">${cartItemName(it.kind,it.key)}</p><span class="cart-item-price">${price} ${content[lang].jod} × ${it.qty}</span></div><div class="cart-item-controls"><div class="cart-item-qty"><button type="button" data-cart-qty="minus" data-kind="${it.kind}" data-key="${it.key}" aria-label="-">−</button><span>${it.qty}</span><button type="button" data-cart-qty="plus" data-kind="${it.kind}" data-key="${it.key}" aria-label="+">＋</button></div><button type="button" class="cart-item-remove" data-cart-remove data-kind="${it.kind}" data-key="${it.key}">${content[lang].removeItem}</button></div></div>`;
}
function renderCart(){
  const totals=cartTotals();
  const fab=$("#cartToggle");
  if(fab)fab.hidden=cart.length===0;
  const countEl=$("#cartCount");if(countEl)countEl.textContent=totals.count;
  const itemsHtml=cart.length?cart.map(cartRowHtml).join(""):`<p class="cart-drawer-empty">${content[lang].cartEmpty}</p>`;
  const itemsEl=$("#cartItems");if(itemsEl)itemsEl.innerHTML=itemsHtml;
  const drawerTotal=$("#cartDrawerTotal");if(drawerTotal)drawerTotal.textContent=`${totals.total} ${content[lang].jod}`;
  const deliveryRow=$("#cartDeliveryRow");if(deliveryRow)deliveryRow.textContent=`${totals.delivery} ${content[lang].jod}`;
  const orderList=$("#orderCartList");
  if(orderList)orderList.innerHTML=cart.length?cart.map(it=>`<div class="selected-preview"><img src="${cartItemImage(it.kind,it.key)}" alt="" width="60" height="60"><div><small>${cartItemName(it.kind,it.key)}</small><strong>${lang==="ar"?"الكمية":"Qty"}: ${it.qty}</strong><span>${cartUnitPrice(it.kind,it.key)*it.qty} ${content[lang].jod}</span></div></div>`).join(""):`<p class="order-cart-list-empty">${content[lang].cartEmpty}</p>`;
  const orderTotal=$("#orderTotal");if(orderTotal)orderTotal.textContent=`${totals.total} ${content[lang].jod}`;
  $$('.package-card[data-package]').forEach(c=>c.classList.toggle("selected",cart.some(it=>it.kind==="package"&&it.key===c.dataset.package)));
  $$('.choose-package').forEach(b=>b.setAttribute("aria-pressed",String(cart.some(it=>it.kind==="package"&&it.key===b.dataset.value))));
  $$('.add-single').forEach(b=>{
    const item=cart.find(it=>it.kind==="single"&&it.key===b.dataset.single);
    b.classList.toggle("in-cart",!!item);
...
if(cartItemsEl)cartItemsEl.addEventListener("click",e=>{
  ...
  if(qtyBtn){
    const kind=qtyBtn.dataset.kind,key=qtyBtn.dataset.key;
    const existing=cart.find(it=>it.kind===kind&&it.key===key);
    if(existing)setCartQty(kind,key,existing.qty+(qtyBtn.dataset.cartQty==="plus"?1:-1));
    return;
  }
  const removeBtn=e.target.closest("[data-cart-remove]");
  if(removeBtn)removeFromCart(removeBtn.dataset.kind,removeBtn.dataset.key);
});
$('#routineOrder').addEventListener('click',e=>{e.preventDefault();addToCart('package',activePlan,1)});
...
$$('.choose-package').forEach(b=>b.addEventListener('click',()=>{
  addToCart('package',b.dataset.value,1);
  heroPreviewType=b.dataset.value;syncHero();
  renderPlan(b.dataset.value);
}));
...
$("#orderForm").addEventListener("submit", e => {
  ...
  if (cart.length === 0) {
    err.textContent = content[lang].emptyCartError;
    return;
  }
  if (!f.checkValidity()) {
  ...
  err.textContent = "";
  const message = buildOrderMessage(cart, new FormData(f));
...
$$('[data-select-set]').forEach(b=>b.addEventListener('click',()=>{heroPreviewType=b.dataset.selectSet;syncHero();addToCart('package',b.dataset.selectSet,1);renderPlan(b.dataset.selectSet)}));
